...
//mostrar los productos a comprar 
function mostrar_orden() {
    ...
    <th>cantidad</th>
    <th>nombre</th>
    <th class= 'remover-nombre'>precio.Unidad</th>
    <th>subtotal</th>
    </tr>`
    ...
    comprados.forEach(producto => {
        ...
        let acumular = producto.contador * producto.price;

        productos_proceso_de_compra += `
        <tr>
        <td>${producto.contador}</td>
        <td>${producto.name}</td>
        <td class='remover-nombre'>${producto.price}</td>
        <td>${acumular}</td>
        </tr>
        `
        acumulador += acumular 
    ...
    document.getElementById("order-total").innerHTML = `Total $${acumulador}`
}

mostrar_orden();
// funcion de uso de api para el buen uso de seleccion de pais
/*async function fetch_paises(paisCode) {
    
    
    let resultado = await (await fetch('https://restcountries.com/v3.1/all')).json();
...
async function obtenerPaises(codepais) {
 ...
 try{
     ...
     // let departamento = datos.geonames.find(dep => dep.toponymName === "Departamento de Cundinamarca") 
     // if(departamento) {
     //     return obtenerCiudades(departamento.geonameId, departamento.toponymName)
     // }
...
obtener_direccion()
 ...
 /*function informacion(direccion) {

    if(direccion.address.id === "") {
        console.log("usuario sin direccion");
    }else{
        console.log("usuario para actualizar")
        console.log(direccion)
    }
